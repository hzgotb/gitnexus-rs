...
fn configureCloneChild(child: *std.process.Child) void {
...
fn defaultCloneRunner(allocator: Allocator, remote: []const u8, target_abs: []const u8) !void {
    const argv = [_][]const u8{ "git", "clone", remote, target_abs };
    var child = std.process.Child.init(&argv, allocator);
    configureCloneChild(&child);
...
pub const testing = struct {
    pub const CloneRunnerType = @TypeOf(clone_runner);

    pub const ParsedArgsView = struct {
        src_raw: ?[]const u8 = null,
        dest_name_arg: ?[]const u8 = null,
        from_source_remote: ?[]const u8 = null,
    };

    pub fn parseArgs(args: []const []const u8) !ParsedArgsView {
        const parsed = try parseAddRepoArgs(args);
        return .{
            .src_raw = parsed.src_raw,
            .dest_name_arg = parsed.dest_name_arg,
            .from_source_remote = parsed.from_source_remote,
        };
    }

    pub fn setCloneRunner(runner: CloneRunnerType) CloneRunnerType {
        const original_runner = clone_runner;
        clone_runner = runner;
        return original_runner;
    }

    pub fn configureCloneChildForTest(child: *std.process.Child) void {
        configureCloneChild(child);
    }

    pub fn ensureRemoteSourceReadyForTest(
        allocator: Allocator,
        cwd_abs: []const u8,
        home_dir: []const u8,
        src_raw: []const u8,
